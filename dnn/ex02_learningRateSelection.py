...
sgd_median = SGD(lr=0.01, # 填入一個數字, ex: 0.01
				momentum=0.0,decay=0.0,nesterov=False)
...
sgd_small = SGD(lr=0.001, # 填入一個數字, ex: 0.001
				momentum=0.0,decay=0.0,nesterov=False)
...
model_lucky = Sequential()
model_lucky.add(Dense(128, input_dim=200))
model_lucky.add(Activation('sigmoid'))
model_lucky.add(Dense(256))
model_lucky.add(Activation('sigmoid'))
model_lucky.add(Dense(5))
model_lucky.add(Activation('softmax'))

''' set the learning rate of SGD optimizer to 0.001 '''
# (Do!) 設定一個比 sgd_median還小 learning rate
sgd_lucky = SGD(lr=0.0519, # 填入一個數字, ex: 0.001
				momentum=0.0,decay=0.0,nesterov=False)

model_lucky.compile(loss= 'categorical_crossentropy',
              optimizer=sgd_lucky,
              metrics=['accuracy'])

history_lucky = model_lucky.fit(X_train, Y_train,
						batch_size=batch_size,
						epochs=epochs,
						verbose=0,
						shuffle=True,
...
loss_lucky = history_lucky.history.get('loss')
acc_lucky = history_lucky.history.get('acc')
...
plt.plot(range(len(loss_lucky)), loss_lucky,label='lr=0.0519')
plt.title('Loss')
...
plt.plot(range(len(acc_lucky)), acc_lucky,label='lr=0.0519')
plt.title('Accuracy')
